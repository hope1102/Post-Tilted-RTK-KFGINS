...
int main(int argc, char *argv[]) {
    ...
    try {
        imupath    = config["imupath"].as<std::string>();// IMU数据路径
        gnsspath   = config["gnsspath"].as<std::string>();// GNSS数据路径
        outputpath = config["outputpath"].as<std::string>();// 输出路径
    } catch (YAML::Exception &exception) {
    ...
    try {
        imudatalen  = config["imudatalen"].as<int>();// 数据长度
        imudatarate = config["imudatarate"].as<int>();// 采样率(Hz)
        starttime   = config["starttime"].as<double>();// 处理起始时间(s)
        endtime     = config["endtime"].as<double>();// 处理结束时间(s)
    } catch (YAML::Exception &exception) {
    ...
    FileSaver navfile(outputpath + "/KF_GINS_Navresult.nav", nav_columns, FileSaver::TEXT);//	导航结果：时间+位置(经纬高)+速度(北东地)+姿态(横滚俯仰航向)
    FileSaver imuerrfile(outputpath + "/KF_GINS_IMU_ERR.txt", imuerr_columns, FileSaver::TEXT);//IMU误差估计：陀螺零偏、加速度计零偏、比例因子
    FileSaver stdfile(outputpath + "/KF_GINS_STD.txt", std_columns, FileSaver::TEXT);//状态估计的标准差（协方差矩阵对角线开根号）
    ...
    } while (imu_cur.time < starttime); // 跳过starttime之前的IMU数据

    GNSS gnss;
    do {
        gnss = gnssfile.next();
    } while (gnss.time <= starttime);  // 跳过starttime之前的GNSS数据
